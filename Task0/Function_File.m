...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  y =jacobian([x1_dot,x2_dot],[x1,x2]);
  z = length(x_2);
  for k = 1 : z
  
  jacobian_matrices{k}  = double(subs( y, [ x1, x2 ], [ solutions(k),solutions(z+k) ] ));
...
function [eigen_values stability] = check_eigen_values(x_1, x_2, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    for z = 1 : length(eigen_values{k})
    if ( real(eigen_values{k}(z,1)) > 0 )
      flag = 0;
      break
    endif
    endfor
