...
function dy = RW_pendulum_dynamics(y, m1, m2, l1, wr, g, u)
  ##u=2*u
  lc1=(m2*l1+m1*(l1/2))/(m1+m2);

  I1= (m1*l1^2)/12;
  I2=(m2*wr^2)/2;
  dy(1,1) = y(2);
  dy(2,1) = ((I2*(m1*lc1+m2*l1)*g*sin(y(1)))-u*(m1*lc1^2+m2*l1^2+I1+I2))/-(I1*I2+m1*lc1^2*I2+m2*l1^2*I2);
  dy(3,1) = y(4);
  dy(4,1) = ((m1*lc1*m2*l1)*g*sin(y(1))-u)/(m1*lc1^2+m2*l1^2+I1);
endfunction
...
function [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1, wr, g)
  lc1=(m2*l1+m1*(l1/2))/(m1+m2);
  I1=(m1*l1^2)/12;
  I2=(m2*wr^2)/2; ;  # Moment of Inertia of pendulum bar
   # Moment of Inertia of reaction wheel
  A = [0 1 0 0;(I2*(m1*lc1+m2*l1)*g)/-(I1*I2+m1*lc1^2*I2+m2*l1^2*I2) 0 0 0;0 0 0 1;-(m1*lc1*m2*l1)*g/(m1*lc1^2+m2*l1^2+I1) 0 0 0];
  B = [0  ;(m1*lc1^2+m2*l1^2+I1+I2)/(I1*I2+m1*lc1^2*I2+m2*l1^2*I2);0;-1/(m1*lc1^2+m2*l1^2+I1)];
endfunction
...
function [t,y] = pole_place_RW_pendulum(m1, m2, l1, wr, g, y_setpoint, y0)
  ...
  eigs = [-4 -2.5 -3 -1.5];                             ## Initialise desired eigenvalues
  K = place(A,B,eigs);
  tspan = 0:0.1:10;
...
function [t,y] = lqr_RW_pendulum(m1, m2, l1, wr, g, y_setpoint, y0)
  ...
  Q = [50 0 0 0;0 50 0 0;0 0 50 0;0 0 0 50];                   ## Initialise Q matrix
  R = [1 ];                   ## Initialise R 
  K =lqr(A,B,Q,R)  
